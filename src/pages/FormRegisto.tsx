...
const FormRegisto = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      handleGetInfo(params.inte);

      e.currentTarget.reset();
    } catch (error) {}
  };

  const handleGetInfo = (info: string) => {
    let url = "";
    let namePDF = "";
    switch (info) {
      case "MAQUINAS":
        url = `${window.location.origin}/APARATOLOGÍA ESTÉTICA.pdf`;
        namePDF = "APARATOLOGÍA ESTÉTICA.pdf";
        break;
      case "PRODUCTO":
        url = `${window.location.origin}/Catálogo línea cosbiome.pdf`;
        namePDF = "Catálogo línea cosbiome.pdf";
        break;
      case "COSMETOLOGIA":
        url = `${window.location.origin}/Oferta académica.pdf`;
        namePDF = "Oferta académica.pdf";
        break;
      case "CURSO":
        url = `${window.location.origin}/Oferta académica.pdf`;
        namePDF = "Oferta académica.pdf";
  ...
  return (
    <>
      ...
      <div className="container mb-3 mt-5">
        <button onClick={() => handleGetInfo(params.inte)} className="btn">
          omitir
        </button>
      </div>
    </>
